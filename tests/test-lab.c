...
void test_addMultiple(void)
{
  ...
  //Verify the circular nature of the list
  TEST_ASSERT_TRUE(lst_->head->prev->next == lst_->head);
  TEST_ASSERT_TRUE(lst_->head->next->prev == lst_->head);
}


void test_addAfterRemovingAll(void)
{
    populate_list(); 

    // Remove all nodes, freeing their data
    for (int i = 0; i < 5; i++) {
        int *removed_data = (int *)list_remove_index(lst_, 0); // Remove node at index 0
        TEST_ASSERT_NOT_NULL(removed_data);                   // Ensure data is valid
        free(removed_data);                                   // Free the removed data
    }

    // Verify the list is empty
    TEST_ASSERT_TRUE(lst_->size == 0);

    // Add a new node after removing all
    int *data = alloc_data(16); 
    list_add(lst_, data);

    // Verify the new node was added successfully
    TEST_ASSERT_TRUE(lst_->size == 1);
    TEST_ASSERT_TRUE(*((int *)lst_->head->next->data) == 16);

    // Verify the circular structure
    TEST_ASSERT_TRUE(lst_->head->next == lst_->head->prev);

    // Cleanup: Free the remaining node's data
    list_destroy(&lst_);
}
...
void test_removeFromEmptyList(void)
{
  ...
  // Verify the sentinel node points to itself
  TEST_ASSERT_TRUE(lst_->head->next == lst_->head);
  TEST_ASSERT_TRUE(lst_->head->prev == lst_->head);
}

void test_removeFromListWithOneNode(void)
{
  //Remove from a list with one node
  int *data = alloc_data(16);
  list_add(lst_, data);
  TEST_ASSERT_TRUE(lst_->size == 1);

  //Verify if removed correctly
  void *rval = list_remove_index(lst_, 0);
  TEST_ASSERT_TRUE(rval == data);
  free(rval);
  TEST_ASSERT_TRUE(lst_->size == 0);
...
int main(void) {
  ...
  RUN_TEST(test_addAfterRemovingAll);
  RUN_TEST(test_removeFromEmptyList);
  RUN_TEST(test_removeFromListWithOneNode);
  return UNITY_END();
}
